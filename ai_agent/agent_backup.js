...
export async function startAIAgent({ url }) {
    ...
    IMPORTANT: After fetching HTML content, you MUST save the raw HTML and extract data using terminal commands. 
    Then analyze the structure and recreate the website by writing NEW HTML, CSS, and JS files in the ./clone/{hostname}/ directory.
    Use terminal commands to create directories, save files, download images, and organize the data.
    ...
    IMPORTANT: Use executeCommand to save HTML content to files and create directory structures.
    Save the fetched HTML to website_data/{hostname}/raw.html using terminal commands.
    ...
    - executeCommand(command: string): Executes terminal/shell commands on the user's machine. Use this to create directories, save HTML to files, download images, extract data, and manage the file system.
    - writeFileContent(filePath: string, content: string): Writes content to a file at the specified path. Creates directories if needed. Use format: "filepath|content".

    IMPORTANT NOTES:
    - Use executeCommand to create directories, save HTML files, and download images with wget/curl
    - For writeFileContent, use format: "filepath|content" where filepath includes the full path.
    - Always call getHTMLContent first, then use executeCommand to save the HTML and process the website data.

    Your Task: Clone a website by:
    1. Fetching the HTML content
    2. Using terminal commands to save HTML, create directory structure, and download assets
    3. Analyzing the HTML structure and extracting CSS/JS
    4. Writing new HTML/CSS/JS files based on the extracted structure
    5. Saving the cloned website in ./clone/{hostname} directory
    6. Using terminal commands to organize and finalize the clone
    7. Providing a summary of what was cloned and where it was saved
    ...
    ASSISTANT: { "step": "THINK", "content": "Great! I got the HTML content. Now I need to save this data and create directory structure using terminal commands." }
    ASSISTANT: { "step": "TOOL", "tool_name": "executeCommand", "input": "mkdir -p website_data/example.com" }
    USER: { "step": "OBSERVE", "content": "Directory created successfully" }
    ASSISTANT: { "step": "THINK", "content": "Now I need to save the HTML content to a file. I'll use the HTML content from memory." }
    ASSISTANT: { "step": "TOOL", "tool_name": "writeFileContent", "input": "website_data/example.com/raw.html|<!DOCTYPE html><html>..." }
    USER: { "step": "OBSERVE", "content": "Successfully wrote file: website_data/example.com/raw.html" }
    ASSISTANT: { "step": "THINK", "content": "Perfect! Now I have the HTML saved. I need to analyze it and create a new HTML file for the clone." }
    ...
    ASSISTANT: { "step": "THINK", "content": "Now I should create CSS styles based on the extracted structure." }
    ASSISTANT: { "step": "TOOL", "tool_name": "writeFileContent", "input": "./clone/example.com/styles.css|body { margin: 0; }..." }
    USER: { "step": "OBSERVE", "content": "Successfully wrote file: ./clone/example.com/styles.css" }
    ASSISTANT: { "step": "OUTPUT", "content": "Successfully cloned https://example.com! The website has been recreated in ./clone/example.com with HTML, CSS, and assets." }
  `;
            ...
            if (toollessIterations >= maxToollessIterations) {
                ...
                messages.push({
                    ...
                    content: JSON.stringify({
                        step: 'OBSERVE',
                        content: "Remember to use tools like getHTMLContent, executeCommand, or writeFileContent to make progress. Please call a tool now instead of just thinking."
                    })
                });
